...
def start_listen():
    ...
    Popen('winproxy set --all 127.0.0.1:7654', stdout=open(os.devnull, 'w'), stderr=open(os.devnull, 'w')).communicate()
    time.sleep(0.2)
    Popen('winproxy on', stdout=open(os.devnull, 'w'), stderr=open(os.devnull, 'w')).communicate()
    time.sleep(0.2)
    print("系统代理端口已更改至7654")
    print("开始监听")
    Popen(f'mitmdump -s {script_path}/listen.py -p 7654', stdout=open(os.devnull, 'w'), stderr=open(os.devnull, 'w'))
